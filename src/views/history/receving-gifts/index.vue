      ...
      Modal.confirm({
        ...
        async onOk() {
          ...
          if (value === 'Draft' || value === 'Submit') {
            ...
            showApplyDrawerModal({ applicationId: item.data, loading: false });
            getListDataByCondition();
            return;
          }
          resetFromFields();
...
onMounted(async () => {
  ...
  const listData = await fetchReceivingGiftsList({ userId: authStore.userInfo.sfUserId, newVersion: 'N' });
  listTableLoading.value = false;
...
<template>
  <div>
    ...
    >
      ...
      <a-form ref="receivingGiftFormRef" :model="applyModelRef" :disabled="true">
        <!--政策提示内容-->
        <!--
...
-->
        ...
        <a-row :gutter="24">
          <a-col span="10">
            <a-form-item a-form-item :label="$t('page.receivingGifts.applyForm.giftRecipient')" name="applyName">
              <a-select
...
-->
        ...
        <a-row :gutter="24">
          <a-col span="10">
            <a-form-item :label="$t('page.receivingGifts.applyForm.giftReason_type_label')" name="reasonType">
          ...
          <a-col span="10">
            <a-form-item :label="$t('page.receivingGifts.applyForm.giftDesc_type_label')" name="giftDescType">
        ...
        <a-row v-show="showReasonDesc" :gutter="24">
          <a-col span="24">
            <a-form-item :label="$t('page.receivingGifts.applyForm.giftReason_label')" name="reason">
        ...
        <a-row :gutter="24">
          <a-col span="5">
            <a-form-item :label="$t('page.receivingGifts.applyForm.giftReceivingDate')" name="date">
              <a-date-picker
