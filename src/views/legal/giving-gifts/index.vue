...
const columns: TableColumnsType = [
  { title: '申请者', width: 100, dataIndex: 'sfUserAppliedName', key: 'sfUserAppliedName', fixed: 'left' },
  { title: '申请日期', width: 100, dataIndex: 'createdDate', key: 'createdDate', fixed: 'left' },
  { title: 'CWID', dataIndex: 'sfUserAppliedCwid', key: 'cwsfUserAppliedCwidid', width: 100 },
  { title: '部门', dataIndex: 'department', key: 'department', width: 150 },
  { title: '公司编号', dataIndex: 'employeeLe', key: 'employeeLe', width: 150 },
...
if (listData.data.list !== null && listData.data.list.length > 0) {
...
const getListDataByCondition=async (currentPage=1)=>{
  ...
  if( typeof (searchRangeDate.value)!="undefined"){
    formState.beginDate=searchRangeDate.value[0].format('YYYY-MM-DD')
    formState.endDate=searchRangeDate.value[1].format('YYYY-MM-DD')
  }
  formState.currentPage=currentPage
  const queryResult = await fetchGivingGiftsList(formState)
  console.log(queryResult);
  listDataSource.value=[]
  if (queryResult.data.list !== null && queryResult.data.list.length > 0) {
      listDataSource.value=queryResult.data.list
      listPagination.value.total=queryResult.data.totalCount
      listPagination.value.current=queryResult.data.currPage
      listPagination.value.pageSize=queryResult.data.pageSize
}
        ...
        <a-row :gutter="24">
          <a-col span="10">
            <a-form-item :name="['apply', 'name']" label="申请人" :rules="[{ required: true }]">
              <a-select placeholder="请选择" show-search label-in-value :allow-clear="true"
                :not-found-content="userState.fetching ? null : undefined" :options="applyOptions"
...
-->
          ...
          <a-col span="14">
            <a-form-item :name="['apply', 'cc_name']" label="抄送">
              <a-select mode="multiple" :default-active-first-option="false" :allow-clear="true"
                :not-found-content="userState.fetching ? null : undefined" :filter-option="false" placeholder="请选择"
                :options="ccApplyOptions" @search="onApplySearch"></a-select>
            </a-form-item>
          </a-col>
        </a-row>
      </a-form>
      <a-descriptions :title="$t('page.givingGifts.policy.title')" layout="vertical">
        <a-descriptions-item v-for="(item, index) in $tm(`page.givingGifts.policy.desc_${userInfo.companyCode}`)"
          :key="index" :span="3" :label="item.label">
          <ul>
...
<a-form>
  ...
  <a-row :gutter="24">
    <a-col span="24">
      <a-form-item name="['apply', 'given_company']" label="接收单位名称" :rules="[{ required: true }]">
        <a-select placeholder="请选择" show-search :allow-clear="true"
          :not-found-content="giftCompanyState.fetching ? null : undefined" :default-active-first-option="false"
          :filter-option="false" @search="onGiftCompanySearch" @change="onGiftCompanyChange">
  ...
  <a-row :gutter="24">
    ...
    <a-col span="10">
      <a-form-item name="['apply', 'bayer_coustomer']" label="礼品接收者是否是拜耳现有客户? " :rules="[{ required: true }]">
        <a-select ref="select" v-model:value="formState_apply.apply.bayer_coustomer" @focus="focus"
          @change="handleChange">
      ...
      <a-row :gutter="8">
        <a-col span="6">
          <a-form-item label="申请编号">
            <a-input placeholder="请输入申请编号..." v-model:value="formState.reference"></a-input>
          </a-form-item>
        </a-col>
        <a-col span="4">
          <a-form-item label="公司编号">
            <a-input placeholder="公司编号..." v-model:value="formState.companyCode"></a-input>
          </a-form-item>
        </a-col>
        <a-col span="5">
          <a-form-item label="制表者">
            <a-input placeholder="请输入制表者..." v-model:value="formState.creator"></a-input>
          </a-form-item>
        </a-col>
        <a-col span="5">
          <a-form-item label="申请者">
            <a-input placeholder="请输入申请者..." v-model:value="formState.userName"></a-input>
          </a-form-item>
        </a-col>
        <a-col span="4">
          <a-form-item label="CWID">
            <a-input placeholder="请输入CWID..." v-model:value="formState.cwid"></a-input>
          </a-form-item>
        </a-col>
      </a-row>
      ...
      <a-row :gutter="35">
        <a-col span="4">
          <a-form-item label="类型">
            <a-select ref="select" v-model:value="formState.type"  @change="typeHandleChange()">
        ...
        <a-col span="5">
          <a-form-item label="状态">
            <a-select ref="select" v-model:value="formState.status" @change="statusHandleChange()">
        ...
        <a-col span="10">
          <a-form-item label="申请日期">
            <a-range-picker v-model:value="searchRangeDate" :disabled-date="disabledDate" />
          </a-form-item>
        </a-col>
        <a-col :span="5" style="text-align: right">
          <a-space :size="5">
            <a-button type="primary" html-type="submit" @click="getListDataByCondition()">搜索</a-button>
...
<a-card title="列表">
  ...
  <a-table :columns="columns" :data-source="listDataSource" :scroll="{ x: 1500 }" class="table-list" :loading="listTableLoading"  :pagination="listPagination"   @change="handleTableChange">
