...
const perVerification = (): Promise<void> => {
  return new Promise((resolve, reject) => {
    ...
    if (applyModelRef.headCount !== personArr.length) {
      Modal.warning({
...
-->
          ...
          <a-col span="24">
            ...
            >
              <a-input
                ...
                :placeholder="$t('page.givingHospitality.applyForm.giftReason_label_validation')"
              ></a-input>
            </a-form-item>
          </a-col>
        </a-row>
        ...
        <div v-for="(company, index) in applyModelRef.companyList" :key="company.key">
          <a-row :gutter="24">
            <a-col span="20">
              <a-form-item
                :label="$t('page.givingHospitality.applyForm.giftGivingCompanyName')"
                :name="['companyList', index, 'companyName']"
                :rules="[
                  {
                    required: true,
                    message: $t('page.givingHospitality.applyForm.giftGivingCompanyName_validation'),
                    trigger: 'change'
                  }
                ]"
              >
